...
from my_dassl.data import DatasetWrapper
...
def main(args):
    ...
    backbone_name = backbone_name.replace('-', '')
    output_dir = output_dir.parent / f'{output_dir.name}_{backbone_name}'
    setup_logger(output_dir.as_posix())
    ...
    for train_step in range(1, len(train_dataiter) + 1):
        ...
        data = batch["img"].cuda()
        with torch.no_grad():
            feature = clip_model.encode_image(data)
        feature = feature.cpu()
    ...
    save_dir = os.path.join(
        output_dir,
        cfg.DATASET.NAME, 
        f'num_shots_{cfg.DATASET.NUM_SHOTS}', 
        f'seed_{cfg.SEED}'
    )
