...
parser.add_argument("--wandb_project", type=str, default='bbox4lv', help="Wandb project name")
parser.add_argument(
    "--backbone", 
    type=str, 
    default='', 
...
feature_dir = feature_dir.parent / f'{feature_dir.name}_{backbone_name}'

dataset = args.dataset
dataset_path = os.path.join(f"{feature_dir}", dataset)
...
if args.use_wandb:
    wandb.init(
        ...
        name=f'{args.dataset}' if args.wandb_name is None else args.wandb_name,
        reinit=True,
    )
...
for seed in range(1, args.num_run + 1):
    ...
    fewshot_path = dataset_path / f'num_shots_{args.num_shots}' / f'seed_{seed}'

    train_file = np.load(os.path.join(fewshot_path, "train.npz"))
    fewshot_train_feature, fewshot_train_label = train_file["feature_list"], train_file["label_list"]
    val_file = np.load(os.path.join(fewshot_path, "val.npz"))
    fewshot_val_feature, fewshot_val_label = val_file["feature_list"], val_file["label_list"]
    test_file = np.load(os.path.join(fewshot_path, "test.npz"))
    test_feature, test_label = test_file["feature_list"], test_file["label_list"]

    # search initialization
    search_list = [5e4, 1e4, 5e3, 1e3, 5e2, 1e2, 5e1, 1e1, 1, 1e-1, 5e-1, 1e-2, 5e-2, 1e-3, 5e-3, 1e-4, 5e-4]
    acc_list = []
    ...
    def binary_search(c_left, c_right, seed, step, test_acc_step_list):
        ...
        saveline = "{}, seed {}, {} shot, weight {}, test_acc {:.2f}\n".format(dataset, seed, args.num_shots, c_final, test_acc)
        with open(
            "./report/{}_s{}r{}_details.txt".format(feature_dir, args.num_step, args.num_run),
            "a+",
        ) as writer:
    ...
    for step in range(args.num_step):
        print(
            f"{dataset}, {args.num_shots} Shot, Round {step}: {c_left}/{c_right}",
            flush=True,
        )
...
save_line = "{}, {} Shot, Test acc stat: {:.2f} ({:.2f})\n".format(dataset, args.num_shots, acc_mean, acc_std)
print(save_line, flush=True)
with open(
    "./report/{}_s{}r{}.txt".format(feature_dir, args.num_step, args.num_run),
    "a+",
) as writer:
